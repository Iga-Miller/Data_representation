...
class TNCDataset(Dataset):
    def __init__(self, x, mc_sample_size, window_size):
        super().__init__()
        ...
        self.window_per_sample = (
                (self.T - 2 * self.window_size) // self.sliding_gap
        )
    ...
    def __getitem__(self, ind):
        t = np.random.randint(
            2 * self.window_size,
            self.T - 2 * self.window_size,
            )
        x_t = self.time_series[ind][
            :, t - self.window_size // 2:t + self.window_size // 2
        ]
        X_close = self._find_neighbours(self.time_series[ind], t)
        X_distant = self._find_non_neighbours(self.time_series[ind], t)

        if self.state is None:
            y_t = -1
        else:
            y_t = torch.round(torch.mean(self.state[ind][
                t - self.window_size // 2:t + self.window_size // 2
            ]))

        return x_t, X_close, X_distant, y_t

    def _find_neighbours(self, x, t):
        ...
        for w_t in range(self.window_size, 4 * self.window_size, gap):
            try:
                ...
                for f in range(x.shape[-2]):
                    p = adfuller(np.array(
                        x[f, max(0, t - w_t):min(x.shape[-1], t + w_t)]
                        .reshape(-1,)
                    ))[1]
    ...
    def _find_non_neighbours(self, x, t):
        T = self.time_series.shape[-1]
        if t > T/2:
